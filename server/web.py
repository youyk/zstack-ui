...
from flask.ext.socketio import join_room, leave_room, close_room
import argparse
import utils
import simplejson
import threading
...
class Server(object):
    ...
    def api_query(self, task_id):
        ...
        return task.to_json()
    def is_system_admin(self, accountName, id):
        msg = '''{"org.zstack.header.identity.APIQueryAccountMsg": {"session": {"uuid": "%s"}, "conditions": [{"name": "name", "value": "%s", "op": "="}]}}''' % (id, accountName)
        retMsg = self.api_sync_call(msg)
        log.debug(retMsg)
        retJson = json.loads(retMsg);
        type = retJson['org.zstack.header.identity.APIQueryAccountReply']['inventories'][0]['type']
        return "SystemAdmin" == type
...
def handle_login(data):
    ...
    retJson = json.loads(retMsg);
    if 'org.zstack.header.identity.APILogInReply' in retJson:
        id = retJson['org.zstack.header.identity.APILogInReply']['inventory']['uuid']
        log.debug('id::::::::::::: ' + id)
        join_room(id)
        session['id'] = id
        accountName = simplejson.loads(msg)["org.zstack.header.identity.APILogInByAccountMsg"]["accountName"]
        log.debug(accountName)
        if server.is_system_admin(accountName, id):
            join_room('admin')
